...
% Solve series ODE
xl = linspace(0.001,C.L,51);
xu = linspace(C.L, C.L+100 ,1000);
xmesh = [xl xu];
yinit = [C.Template; C.Template]*1e-3;
sol = bvpinit(xmesh,yinit);
sol = bvp5c(@(x,y,region) SphereDiffusion(x,y,region,C), @bc, sol);

% analytic solution
c1 = (1 - (lambda*C.L + 1).*exp(-lambda*C.L).*sinh(lambda.*xl)./lambda./xl);
c2 = (lambda*C.L*cosh(lambda*C.L) - sinh(lambda*C.L)).*exp(-lambda.*xu)./lambda./xu;
c = [c1 c2];
...
patch = fill([0 1 1 0], [1e-6 1e-6 1 1], 'k');
set(patch,'edgecolor', 'none');
set(patch,'FaceAlpha', 0.10);

% legend([ana1 ana2 ana3],{'R'' = 0.1','R'' = 0.3','R'' = 1'}) 
legend([ana1 ana2 ana3],{"R' = 0.3","R' = 1","R' = 3"}) 

exportgraphics(fig,'fig2f.jpg','Resolution',300)

function dydx = SphereDiffusion(x,y,region,C)
% constants
lambda = sqrt(C.k_Dss*C.RNase/C.D);
A = C.Template*C.RNAP*C.k_p/C.D;
...
switch region
    case 1    % x in [0 1]
        dydx(2) = -2/x*y(2) + lambda^2*y(1) - A;
    case 2    % x in [1 inf]
        dydx(2) =-2/x*y(2) + lambda^2*y(1);
end
end
