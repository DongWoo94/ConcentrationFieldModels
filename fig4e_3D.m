...
C.Template = 150*[exp(100*lambda) 2*exp(200*lambda) 3*exp(300*lambda)] * 1e-9 * 1e-15 * 0.02 * 60; % [=] mol/um3/min; 400nM initial; rejection rate 40%;  1mol/L = 1e-15 mol/um3    1L = 1e15 um3
% 850, 8000, 56000 nM / min
...
% analytic solution
c1 = A.*(1 - (lambda*C.L + 1).*exp(-lambda*C.L).*sinh(lambda.*xl)./lambda./xl);
c2 = A.*(lambda*C.L*cosh(lambda*C.L) - sinh(lambda*C.L)).*exp(-lambda.*xu)./lambda./xu;
c = [c1 c2];

ana = plot(xmesh,c*1e24,"LineWidth",1.5);
end

exportgraphics(fig,'fig4e_3D.jpg','Resolution',300)
